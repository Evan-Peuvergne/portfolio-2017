  ...
  Letter.created = function () {
    ...
    _.each(morphs, (m, j) => {
      for(var i=0; i<m.from.segments.length; i++){
        ...
        tl.to(s.point, .75, { ox: t.point.ox, oy: t.point.oy, ease: ease.elastic, delay: i*.005 }, 0);
        tl.to(s.handleIn, .75, { ox: t.handleIn.ox, oy: t.handleIn.oy, ease: ease.elastic, delay: i*.005 }, 0);
        tl.to(s.handleOut, .75, { ox: t.handleOut.ox, oy: t.handleOut.oy, ease: ease.elastic, delay: i*.005 }, 0);
      }
    });
